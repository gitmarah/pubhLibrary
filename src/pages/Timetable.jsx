...
import { pastPapers } from '../assets/pastPapers.js';
import ModuleExerpt from '../components/ModuleExerpt.jsx';

const Timetable = () => {
    const { search } = useContext(levelContext);
    const list = pastPapers.filter(module => module.title.toLowerCase().includes(search.trim().toLowerCase()) || module.module.toLowerCase().includes(search.trim().toLowerCase()));

    return (
        <>
            <Header calendar="active" placeholder="Search for Past Papers"/>
            <main className='max-w-4xl mx-auto pt-16 pb-20 px-4'>
                { !(list.length === 0) ?
                    ...
                    </fieldset> : <p className='font-montserrat mt-5 text-center'>No past papers for now!</p>
                }
                
            </main>
            <Footer calendar="active" />
        </>
    )
}
